...
def getMetaInfoCommon(exp: oms.MSExperiment, sha256: str=None) -> mzqc.RunQuality:
  ...
  if sha256:
    ...
    chksm = utils.sha256fromfile(exp.getExperimentalSettings().getLoadedFilePath())
...
def getMetricSourceFramesCommon(exp: oms.MSExperiment, config: configparser.ConfigParser) -> List[mzqc.QualityMetric]:
  """
  ...
  for spin, spec in enumerate(exp):
    ...
    if (spec.getMSLevel() == 2):
      ...
      tandem_spectrum_metrics['precursor_int'].append(precursor_int)  # N.B. also different from mzid getPrecursors[0].getIntensity() ? In mzml the latter is usually zero and the precursor_int_dev equals precursor_int
...
def getMetricSourceFramesIdent(pro_ids: List[oms.ProteinIdentification],
          ...
          config: configparser.ConfigParser,
          id_run_idx: int = 0) -> Tuple[Any,Any,Any,Any]:
  """
  ...
  psims: pronto.ontology.Ontology = utils.obtainOntology("psi-ms", config)
  # #pep_ids.sort()
  ...
  if not filtered and pruned:
    psms = None
    all_peptides = pd.DataFrame(utils.iterablePSMfromOpenMS(pep_ids),
            columns=["RT","MZ","c","score","proteinid","sequence","decoy","nativeid"]
          )
    fdr_peptides = all_peptides[all_peptides.score < config['fdr.cutoff']['default']] if has_fdr_equiv else None
  ...
  for pepi in pep_ids:
    if not pepi.empty():
      ...
      if pepi.getHits():
        peptide_seqs.add(pepi.getHits()[0].getSequence().toString())

  for proid in pro_ids:
      protein_evidence_count += len(proid.getHits())
      for p in proid.getHits():
        protein_seqs.add(p.getAccession())
  ...
  return psms, all_peptides, fdr_peptides, metrics
