...
def getSamplingRatios(identification_sequence_metrics: mzqc.QualityMetric) -> List[mzqc.QualityMetric]:
    ...
    sample_rate, sample_rate_counts = np.unique(
        np.unique(identification_sequence_metrics.value['peptide'], return_counts=True)[1], return_counts=True)
...
def getIdentifiedSignalMetrics(tandem_spectrum_metrics: mzqc.QualityMetric,
    ...
    np_prec = np.array([tandem_spectrum_metrics.value['RT'],
                        tandem_spectrum_metrics.value['precursor_mz'],
                        tandem_spectrum_metrics.value['precursor_intensity'],
                        tandem_spectrum_metrics.value['surveyscan_intensity_sum'],
                        tandem_spectrum_metrics.value['surveyscan_intensity_max']])

    # DS-3A reimpl.: median( (surv max / prec int) for all ident. prec )
    id_coord = np.array([identification_accuracy_metrics.value['RT'],
                        identification_accuracy_metrics.value['MZ']])  # TODO make sure intersection is round-proof
    ...
    all_diff = np_id[:, -1 * int(half_id_size):][0] - np_id[:, :int(half_id_size)][
        0]  # the last (half_id_size) many - the first (half_id_size) many
    ...
    densest_id_tic = np.median(
        np_tic[:, (np_id[0, min_start_index] < np_tic[0]) & (np_tic[0] < np_id[0, min_stop_index])][1])
...
def describeIdentifiedPrecursorIntensity(tandem_spectrum_metrics: mzqc.QualityMetric,
